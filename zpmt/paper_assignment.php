...
<?php
	...
	include "conn.php";
?>
	<h2>Assign printers to paper</h2>
	<h3>Select the printer(s), the action and the paper you want to associate or dissociate the printers to.</h3>
			...
			<form action="media_actions.php" method="post">
				...
				<?php
					...
					while($row_printer = $result_printer->fetch_assoc()){
						...
						$row_ribbon = $result_ribbon->fetch_assoc();
		...
		<div class="grid_item" id="box_media_selection">
				<p>
					<select name="action" id="action" required>
						...
						<option value="associate_paper">Associate paper to printer(s)</option>
						<option value="dissociate_paper">Dissociate paper to printer(s)</option>
					</select>
				</p>
				<p>
					<label for="paper"></label>
					<select name="paper_id" id="paper" required>
					...
					<?php
						...
						while($row_paper = $result_paper->fetch_assoc()){
							echo "<option value='" . $row_paper["id"] . "' >" . $row_paper["name"] . "</option>";
						}
						?>
					</select>
				</p>
